...
# LH 동반성장처 공법심의 관련 웹페이지 주소
TARGET_URL = "https://partner.lh.or.kr/deliberate/deliberate.asp"
...
def get_daily_filename():
    ...
    # 파일명 생성
    filename = now.strftime("%y%m%d") + "_LH 공고리스트.json"
...
def scrape_lh():
    """Playwright를 이용해 LH 공고 목록을 수집합니다."""
    notices = []
    ...
    with sync_playwright() as p:
        ...
        try:
            print(f"[{get_now_str()}] LH 홈페이지 접속 중...")
...
def run_monitor():
    """모니터링 핵심 루프"""
    print(f"[{get_now_str()}] LH 공법심의 모니터링을 시작합니다. (키워드: {', '.join(KEYWORDS)})")
    send_telegram_message(f"<b>[LH 모니터링 비서]</b>")
    
    while True:
        try:
            if is_active_time():
                print(f"[{get_now_str()}] LH 공고 확인 중...")
                current_notices = scrape_lh()
                ...
                if current_notices:
                    ...
                    if new_count > 0 or not file_exists:
                        ...
                        print(f"[{get_now_str()}] LH_확인 완료 (신규 발견: {new_count}개 / 파일 갱신 완료)")
                else:
                    print(f"[{get_now_str()}] LH_조건에 맞는 공고가 없거나 데이터를 가져오지 못했습니다.")
            else:
                print(f"[{get_now_str()}] 활성 시간(08:00~18:00)이 아닙니다. 대기 중...")
                
        except Exception as e:
            print(f"[{get_now_str()}] LH_루프 실행 중 에러: {e}")
